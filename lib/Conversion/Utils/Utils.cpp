...
LogicalResult computeBroadcastShape(ConversionPatternRewriter &rewriter,
                                    Location loc, Value inputA, Value inputB,
  ...
  if (rankA == 0 && rankB == 0) {
    ...
    // If we reach here then it means, exactly one of the tensor has a rank 0.
    Value nonZeroRankInput = rankA == 0 ? inputB : inputA;
    resultShape = rankA == 0 ? shapeB : shapeA;
    unsigned nonZeroInputRank = rankA == 0 ? rankB : rankA;
    for (unsigned i = 0; i < nonZeroInputRank; i++) {
      Value sizeDim = rewriter.create<Torch::ConstantIntOp>(
          loc, rewriter.getI64IntegerAttr(i));
      resultShapeValue.push_back(
          rewriter.createOrFold<AtenSizeIntOp>(loc, nonZeroRankInput, sizeDim));
    }
  }
  return success();
}
